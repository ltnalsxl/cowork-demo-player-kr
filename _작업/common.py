...
TC04_BENCH = {
    ...
    'condition': '각 실행 전에 이전 회차가 만든 파일을 OneDrive에서 모두 삭제해 독립 시행으로 '
                 '진행했습니다. 같은 프롬프트, 같은 계정, 같은 데이터 조건입니다. '
                 '노력 수준은 모델 기본값을 따랐습니다(Claude 계열과 자동은 보통, GPT 계열은 매우 높음).',
    'models': [
...
TC01_BENCH = {
    ...
    'condition': '모델과 노력 설정을 같게 두고(자동·보통) 계정만 바꿔 돌렸습니다. '
                 '한쪽은 보낸 메일과 회의, Teams 대화가 모두 0건인 계정이고, '
                 '다른 쪽은 진행 중인 프로젝트 7건과 실제 메일·회의·파일이 있는 계정입니다. '
                 '두 회차 모두 이전 실행이 만든 파일을 미리 지워 독립 시행으로 진행했습니다.',
    'models': [
